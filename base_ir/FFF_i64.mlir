...
!nodes_t = tensor<64xi64>
!gather_nodes_t = tensor<64x1xi64>
!logit_t = tensor<64x11xf32>
!choice_t = tensor<64x11xi64>
!gather_choice_t = tensor<64x11x1xi64>
...
func.func @fff(%in: !in_t, %w1: !w1_t, %w2: !w2_t) -> !out_t {
  ...
  %curr_init = linalg.fill ins(%c0_i64: i64) outs(%curr_empty: !nodes_t) -> !nodes_t

  %rows_empty = tensor.empty() : !choice_t
  %select_init = linalg.fill ins(%c0_i64: i64) outs(%rows_empty: !choice_t) -> !choice_t
  ...
  %s:3 = scf.for %i = %c0 to %d1 step %c1
    ...
    %new_current = linalg.generic {
      ...
      ins(%curr, %scores: !nodes_t, !score_t) outs(%new_curr_empty: !nodes_t) {
        ^bb0(%c: i64, %score: f32, %out: i64):
          %choice = arith.cmpf oge, %score, %c0_f32 : f32
          %choice_i64 = arith.extui %choice : i1 to i64
          %branch = arith.muli %c, %c2_i64 : i64
          %inc = arith.addi %branch, %c1_i64: i64
          %next = arith.addi %inc, %choice_i64: i64
