module @main {
  ...
  util.global private mutable @text : !util.buffer
  util.global private mutable @buffer_len = 0 : index

  func.func private @append_and_grow(%word: !util.buffer) {
    %arg0 = util.global.load @text : !util.buffer
    ...
    util.global.store %new_buffer, @text : !util.buffer
    return
  }

  func.func private @print_text(%stream: !io_stream.handle) {
    %c0_1 = arith.constant 0 : index
    %text = util.global.load @text : !util.buffer
    %text_capacity = util.buffer.size %text : !util.buffer
    %text_size = util.global.load @buffer_len : index
    %slice = util.buffer.slice %text[%c0_1] : !util.buffer{%text_capacity} -> !util.buffer{%text_size}

    io_stream.write.bytes(%stream, %slice, %c0_1, %text_size) : (!io_stream.handle, !util.buffer, index, index) -> ()
    %newline = arith.constant 10 : i8  // \n
    io_stream.write.byte(%stream, %newline) : (!io_stream.handle, i8) -> ()
    return
  }
  ...
  func.func public @detokenize(%ids: tensor<?xi32>) {
    ...
    util.global.store %init, @text : !util.buffer
    ...
    scf.for %arg0 = %c0 to %steps step %c1 {
      ...
      //func.call @print_buffer(%stdout, %word) : (!io_stream.handle, !util.buffer) -> ()
      func.call @append_and_grow(%word) : (!util.buffer) -> ()
      //func.call @print_text(%stdout) : (!io_stream.handle) -> ()
      scf.yield
    }
