module attributes { transform.with_named_sequence } {
  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %forall_grid, %reduction_grid =
      transform.structured.tile_to_forall_op %reduction tile_sizes [0, 0, 4]
        ( mapping = [#gpu.block<x>] )
    ...
    // ===========================================================================
    %padded_reduc, %pad = transform.structured.pad %reduction_grid {copy_back_op = "none", pack_paddings = [1, 0, 1], pad_to_multiple_of = [1, 1], padding_dimensions = [4, 5], padding_values = [0.0 : f16, 0.0 : f16, 0.0 : f16]} : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %4   num_threads [0, 0, 32, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    //%split_forall, %new_fill, %split_reduc, %combiner_op = transform.structured.tile_reduction_using_forall %padded_reduc by num_threads = [0, 0, 4, 1], tile_sizes = [], mapping = [#gpu.thread<y>, #gpu.thread<z>]  : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op, !transform.any_op)
    //transform.apply_patterns to %func {
    //   transform.apply_patterns.tensor.reassociative_reshape_folding
    //   transform.apply_patterns.tensor.merge_consecutive_insert_extract_slice
    //} : !transform.any_op
    //transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    //%fill_3d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<1x1x4xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    //transform.structured.fuse_into_containing_op %fill_3d into %split_forall : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    //transform.structured.fuse_into_containing_op %dequant into %split_forall : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    //transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %dequant = get_producer_of_operand %padded_reduc[1] : (!transform.any_op) -> !transform.any_op
    %outer_forall, %tiled_reduc = transform.structured.tile_to_forall_op %padded_reduc num_threads [0, 0, 4] tile_sizes [](mapping = [#gpu.thread<y>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    %fill_3d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<1x1x4xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    ...
    transform.structured.fuse_into_containing_op %dequant into %outer_forall : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    //transform.iree.map_nested_forall_to_gpu_threads %func_4 workgroup_dims = [128, 4, 1] subgroup_size = 64 : (!transform.any_op) -> ()
