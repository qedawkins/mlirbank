  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %init_or_alloc_op, %more_parallel_fill_op, %more_parallel_op, %combiner_op =
      transform.structured.split_reduction %reduction
        { split_factor = 128, insert_split_dimension = 1, inner_parallel }
    ...
    %forall_grid, %grid_combiner_op =
      transform.structured.tile_to_forall_op %combiner_op tile_sizes [4]
        ( mapping = [#gpu.block<x>] )
    ...
    // Step 1.1: Cannot fuse across the "expand_shape" produced by reduction
    ...
    %grid_more_parallel_op = transform.structured.match ops{["linalg.generic"]}
      attributes{iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<reduction>, #linalg.iterator_type<reduction>, #linalg.iterator_type<parallel>]} in %variant_op : (!transform.any_op) -> !transform.any_op
    %padded_reduc, %pad = transform.structured.pad %grid_more_parallel_op {copy_back_op = "none", pack_paddings = [1, 0, 0], pad_to_multiple_of = [1, 1], padding_dimensions = [2, 3], padding_values = [0.0 : f16, 0.0 : f16, 0.0 : f16]} : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %4   num_threads [32, 1, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    transform.print %variant_op {name = "========= After Tiling copy to threads: "} : !transform.any_op

    // Step 3. Tile the parallel dim.
    // ===========================================================================
    %outer_forall, %tiled_reduc = transform.structured.tile_to_forall_op %combiner_2 num_threads [4] tile_sizes [](mapping = [#gpu.thread<y>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    // Step 4. Tile the reduction dim.
    // ===========================================================================
    %dequant = get_producer_of_operand %padded_reduc[1] : (!transform.any_op) -> !transform.any_op
    %loop_reduc, %new_for = transform.structured.tile_to_scf_for %padded_reduc [0, 8] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.apply_patterns to %func {
    ...
    // Step 5. Tile the parallel dim.
    // ===========================================================================
    %tiled_dequant = get_producer_of_operand %loop_reduc[1] : (!transform.any_op) -> !transform.any_op
    %partial_forall, %partial_reduc = transform.structured.tile_to_forall_op %loop_reduc num_threads [4, 0, 0, 128] tile_sizes [](mapping = [#gpu.thread<y>, #gpu.thread<x>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %fill_2d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<4x128xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    %fill_forall, %tiled_fill = transform.structured.tile_to_forall_op %fill_2d num_threads [4, 128] tile_sizes [](mapping = [#gpu.thread<y>, #gpu.thread<x>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    transform.iree.map_nested_forall_to_gpu_threads %func_m workgroup_dims = [128, 4, 1] subgroup_size = 64 : (!transform.any_op) -> ()
    ...
    // Step 9. Post-bufferization vector distribution with rank-reduction.
    // ===========================================================================
    ...
    transform.print %variant_op_2 {name = "========= After SPIRVVectorize: "} : !transform.any_op
  }
}
