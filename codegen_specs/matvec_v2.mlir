module attributes { transform.with_named_sequence } {
  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %4   num_threads [4, 8, 16, 1] tile_sizes [](mapping = [#gpu.thread<linear_dim_3>, #gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %partial_forall, %partial_reduc = transform.structured.tile_to_forall_op %loop_reduc num_threads [4, 8, 0, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %fill_2d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<4x8x16xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    %fill_forall, %tiled_fill = transform.structured.tile_to_forall_op %fill_2d num_threads [4, 8, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
