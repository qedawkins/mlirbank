module {
  ...
  ^bb0(%arg0: !transform.any_op):
    ...
    %forall_op, %tiled_op = transform.structured.tile_to_forall_op %trailing   num_threads [] tile_sizes [4, 16](mapping = [#gpu.block<y>, #gpu.block<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    //%tiled_linalg_op, %loops = transform.structured.tile_to_scf_for %fused_op[0, 0, 0, 1] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    //%decomp = transform.structured.decompose %tiled_linalg_op : (!transform.any_op) -> !transform.any_op
    //apply_patterns to %arg0 {
    //  transform.apply_patterns.linalg.tiling_canonicalization
    //  transform.apply_patterns.iree.fold_fill_into_pad
    //  transform.apply_patterns.scf.for_loop_canonicalization
    ...
    // Old Step 4. Tile along filter dimensions.
    ...
    %forall_op_15, %tiled_op_15 = transform.structured.tile_to_forall_op %tiled_linalg_op   num_threads [0, 16, 4] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    apply_patterns to %arg0 {
      transform.apply_patterns.linalg.tiling_canonicalization
      transform.apply_patterns.iree.fold_fill_into_pad
      transform.apply_patterns.scf.for_loop_canonicalization
    ...
    %forall_op_16, %tiled_op_16 = transform.structured.tile_to_forall_op %new_fill   num_threads [0, 16, 4] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    %forall_op_17, %tiled_op_17 = transform.structured.tile_to_forall_op %tiled_op   num_threads [0, 16, 4] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    transform.iree.map_nested_forall_to_gpu_threads %16 workgroup_dims = [64, 1, 1] warp_dims = [1, 1, 1] subgroup_size = 64 : (!transform.any_op) -> ()
