  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %init_or_alloc_op, %more_parallel_fill_op, %more_parallel_op, %combiner_op =
      transform.structured.split_reduction %reduction
        { split_factor = 128, insert_split_dimension = 1 }
    ...
    %grid_more_parallel_op = transform.structured.match ops{["linalg.generic"]}
      attributes{iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>, #linalg.iterator_type<reduction>, #linalg.iterator_type<reduction>]} in %variant_op : (!transform.any_op) -> !transform.any_op
    ...
    %tiled_dequant = get_producer_of_operand %padded_reduc[1] : (!transform.any_op) -> !transform.any_op
    %outer_forall, %tiled_reduc = transform.structured.tile_to_forall_op %combiner_2 num_threads [1] tile_sizes [](mapping = [#gpu.thread<y>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    %fill_1d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<1xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    %fill_2d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<1x128xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    ...
    %fused_reduc, %new_forall = transform.structured.fuse_into_containing_op %padded_reduc into %outer_forall : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    transform.structured.fuse_into_containing_op %fill_2d into %outer_forall : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()

    //%fill_2d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<1x128xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    //%fill_forall, %tiled_fill = transform.structured.tile_to_forall_op %fill_2d num_threads [4, 32, 2] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    //transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()

    // Step 3. Tile the reduction dim.
    // ===========================================================================
    %dequant = get_producer_of_operand %fused_reduc[1] : (!transform.any_op) -> !transform.any_op
    %loop_reduc, %new_for = transform.structured.tile_to_scf_for %fused_reduc [0, 0, 8] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.apply_patterns to %func {
