  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %init_or_alloc_op, %more_parallel_fill_op, %more_parallel_op, %combiner_op =
      transform.structured.split_reduction %reduction
        split_factor = [4, 16] insert_split_dimension = [1, 3] inner_parallel = [true, false]
    ...
    %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %4   num_threads [8, 4, 8, 1] tile_sizes [](mapping = [#gpu.thread<linear_dim_3>, #gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %partial_forall, %partial_reduc = transform.structured.tile_to_forall_op %loop_reduc num_threads [4, 4, 0, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %fill_2d = transform.structured.match ops{["linalg.fill"]} filter_result_type = tensor<4x4x16xf16> in %variant_op : (!transform.any_op) -> !transform.any_op
    %fill_forall, %tiled_fill = transform.structured.tile_to_forall_op %fill_2d num_threads [4, 4, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_2>, #gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    transform.iree.map_nested_forall_to_gpu_threads %func_m workgroup_dims = [64, 4, 1] subgroup_size = 64 : (!transform.any_op) -> ()
