...
^bb0(%arg0: !pdl.operation):
  ...
  %forall_op, %tiled_op = transform.structured.tile_to_forall_op %first   num_threads [] tile_sizes [1, 16](mapping = [#gpu.block<y>, #gpu.block<x>])
  ...
  %8 = transform.structured.pad %tiled_linalg_op {pack_paddings = [0, 1, 1], padding_dimensions = [0, 1, 2], padding_values = [0 : i8, 0 : i8, 0 : i32]}
  ...
  %12 = get_producer_of_operand %8[0] : (!pdl.operation) -> !pdl.operation
  %13 = get_producer_of_operand %8[1] : (!pdl.operation) -> !pdl.operation
  %14 = transform.structured.rewrite_in_destination_passing_style %13 : (!pdl.operation) -> !pdl.operation
  %forall_op_2, %tiled_op_3 = transform.structured.tile_to_forall_op %12   num_threads [0, 16, 16] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_4, %tiled_op_5 = transform.structured.tile_to_forall_op %14   num_threads [0, 0, 16, 16] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %rest_1   num_threads [0, 16, 16] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_8, %tiled_op_9 = transform.structured.tile_to_forall_op %9   num_threads [0, 16, 16] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_10, %tiled_op_11 = transform.structured.tile_to_forall_op %8   num_threads [1, 1, 8] tile_sizes [](mapping = [#gpu.warp<z>, #gpu.warp<y>, #gpu.warp<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  ...
  //transform.iree.map_nested_forall_to_gpu_threads %19 workgroup_dims = [256, 1, 1] warp_dims = [8, 1, 1] : (!pdl.operation) -> ()
