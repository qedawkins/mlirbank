module {
  ...
  ^bb0(%arg0: !transform.any_op):
    ...
    //%fused_pad, %new_containing_op_2 = transform.structured.fuse_into_containing_op %pad into %forall_op : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %padded = transform.structured.pad %fused_op {copy_back = false, pack_paddings = [1, 0, 1], padding_dimensions = [0, 1, 2, 3, 4, 5], padding_values = [0 : i8, 0 : i8, 0 : i32]} : (!transform.any_op) -> !transform.any_op
    apply_patterns to %arg0 {
      transform.apply_patterns.linalg.tiling_canonicalization
      transform.apply_patterns.iree.fold_fill_into_pad
      transform.apply_patterns.scf.for_loop_canonicalization
    ...
    //%forall_op_18, %tiled_op_18 = transform.structured.tile_to_forall_op %inp_copy   num_threads [3, 6, 0] tile_sizes [](mapping = [#gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    //%thread_fused_pad, %new_containing_op_3 = transform.structured.fuse_into_containing_op %fused_pad into %forall_op_18 : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %concrete = transform.apply_registered_pass "iree-codegen-concretize-pad-result-shape" to %12 : (!transform.any_op) -> !transform.any_op
    apply_patterns to %concrete {
      transform.apply_patterns.iree.swap_tensor_pad
    } : !transform.any_op
    %concrete_2 = transform.apply_registered_pass "iree-codegen-concretize-pad-result-shape" to %concrete : (!transform.any_op) -> !transform.any_op
    transform.iree.apply_cse %arg0 : !transform.any_op
    apply_patterns to %concrete_2 {
      transform.apply_patterns.iree.fold_reshape_into_tensor_hal_interface
      transform.apply_patterns.linalg.fold_unit_extent_dims_via_slices
      transform.apply_patterns.vector.cast_away_vector_leading_one_dim
    } : !transform.any_op
    %13 = transform.structured.vectorize %concrete_2 {vectorize_padding, vectorize_nd_extract} : (!transform.any_op) -> !transform.any_op
