module attributes { transform.with_named_sequence } {
  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %tiled_reduc, %loop = transform.structured.tile %padded_reduc [4] : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    // ===========================================================================
    %dequant2 = get_producer_of_operand %tiled_reduc[1] : (!transform.any_op) -> !transform.any_op
    %outer_forall, %single_reduc = transform.structured.tile_to_forall_op %tiled_reduc num_threads [4] tile_sizes [](mapping = [#gpu.thread<y>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
