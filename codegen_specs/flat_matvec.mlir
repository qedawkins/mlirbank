module attributes { transform.with_named_sequence } {
  ...
  ^bb1(%variant_op: !transform.any_op):
    ...
    %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %4   num_threads [16, 16] tile_sizes [](mapping = [#gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
    ...
    %dequant = get_producer_of_operand %padded_reduc[1] : (!transform.any_op) -> !transform.any_op
    %outer_forall, %tiled_reduc = transform.structured.tile_to_forall_op %padded_reduc num_threads [4] tile_sizes [](mapping = [#gpu.thread<y>])  : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%variant_op) : (!transform.any_op) -> ()
