module attributes { transform.with_named_sequence } {
  ...
  ^bb0(%arg0: !transform.any_op):
    ...
    %conv, %trailing, %pixel_shuffle = transform.split_handle %generics : (!transform.any_op) -> (!transform.any_op, !transform.any_op, !transform.any_op)
    ...
    //===------------------------------------------------------===
    %forall_op, %tiled_pixel_shuffle_op = transform.structured.tile_to_forall_op %pixel_shuffle   num_threads [] tile_sizes [2, 64](mapping = [#gpu.block<y>, #gpu.block<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    %fused_trailing_op, %forall_op_2 = transform.structured.fuse_into_containing_op %trailing into %forall_op : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    %fused_conv, %forall_op_4 = transform.structured.fuse_into_containing_op %conv into %forall_op_3 : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    %fused_fill, %forall_op_5 = transform.structured.fuse_into_containing_op %fill into %forall_op_4 : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    //===------------------------------------------------------===
    %padded_conv, %conv_input_pad = transform.structured.pad %fused_conv {copy_back = false, pack_paddings = [1, 0, 1], pad_to_multiple_of = [1, 1, 1], padding_dimensions = [0, 1, 2], padding_values = [0 : i8, 0 : i8, 0 : i32]} : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()
    ...
    // Step 4. Create the input copy.
    //===------------------------------------------------------===
    %forall_op_6, %tiled_dps_copy = transform.structured.tile_to_forall_op %dps_copy   num_threads [4, 11, 1] tile_sizes [](mapping = [#gpu.linear<z>, #gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()

    //===------------------------------------------------------===
    // Step 5. Tile the filter dimensions.
    //===------------------------------------------------------===
    ...
    //===------------------------------------------------------===
    %forall_op_7, %tiled_inner_gemm = transform.structured.tile_to_forall_op %inner_gemm   num_threads [2] tile_sizes [](mapping = [#gpu.warp<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()

    //===------------------------------------------------------===
    // Step 7. Tile to threads (SIMT).
    //===------------------------------------------------------===
    %tiled_fill = transform.structured.match ops{["linalg.fill"]} in %arg0 : (!transform.any_op) -> !transform.any_op
    %forall_op_8, %fill_l2 = transform.structured.tile_to_forall_op %tiled_fill   num_threads [2, 4, 8] tile_sizes [](mapping = [#gpu.linear<z>, #gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()
    %forall_op_9, %pixel_shuffle_l2 = transform.structured.tile_to_forall_op %tiled_pixel_shuffle_op   num_threads [2, 4, 8] tile_sizes [](mapping = [#gpu.linear<z>, #gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    %trailing_l2, %forall_op_10 = transform.structured.fuse_into_containing_op %fused_trailing_op into %forall_op_9 : (!transform.any_op, !transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()
    ...
    //%22 = apply_registered_pass "iree-codegen-gpu-reduce-bank-conflicts" to %21 : (!transform.any_op) -> !transform.any_op
