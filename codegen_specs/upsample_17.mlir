module attributes { transform.with_named_sequence } {
  ...
  ^bb0(%arg0: !transform.any_op):
    ...
    //===------------------------------------------------------===
    %forall_op, %tiled_op = transform.structured.tile_to_forall_op %upsample   num_threads [] tile_sizes [2, 32](mapping = [#gpu.block<y>, #gpu.block<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    ...
    //===------------------------------------------------------===
    %forall_op_threads, %tiled_op_threads = transform.structured.tile_to_forall_op %tiled_op   num_threads [2, 32, 1] tile_sizes [](mapping = [#gpu.linear<z>, #gpu.linear<y>, #gpu.linear<x>]) : (!transform.any_op) -> (!transform.any_op, !transform.any_op)
    transform.include @cleanup failures(propagate) (%arg0) : (!transform.any_op) -> ()
